...
{
    public partial class frm_InvoiceHeader_store : Form
    {
        ...
        void get_store()
        {
            ...
            dr_store_to.IntializeData(stores.ToList(), "store_name", "id");
            dr_store.IntializeData(stores.ToList(), "store_name", "id");
        ...
        void getdata()
        {
            ...
            if (invoiceHeader.id != 0)
            {
                ...
                lp_titel.Text = " تعديل بيان تحويل من مخزن الي مخزن";
                tx_DateAdd.Value = invoiceHeader.DateAdd;
                btn_print.Visible = true;
                updateid = true;
                var q = db.store_logs.Where(x => x.Source_Id == invoiceHeader.id).ToList();
                invo_store_id = q.Where(x => x.ItemQty_in > 0).FirstOrDefault().store_id;                
                dr_store_to.SelectedValue = invo_store_id;

                btn_delete.Visible = Session.User_setting().delete_invoice_to_stroe;
                btn_save.Visible = Session.User_setting().update_invoice_to_stroe;
            }
            else
            {
                lp_titel.Text = "بيان جديد تحويل من مخزن الي مخزن ";
                btn_delete.Visible = false;
                btn_print.Visible = false;
                btn_save.Visible = Session.User_setting().add_invoice_to_stroe;
                btn_new.Visible = Session.User_setting().add_invoice_to_stroe;
            }
            tx_ItemName.Focus();
        }
        void setdata()
        {
            ...
            invoiceHeader.id_invoice_type = 3;
        ...
        bool valid()
        {
            ...
            int error = 0;
            if (dr_store_to.SelectedIndex == -1)
            {
                errorProvider1.SetError(dr_store_to, "يجب اختيار مخزن");
                error++;
            }
            if (dr_store_to.SelectedIndex != -1)
            {
                for (int i = 0; i < gv_InvoiceDetails.Rows.Count; i++)
                {
                    ...
                    {
                        gv_InvoiceDetails.Rows[i].Cells[nameof(Column2)].Style.BackColor= Color.Red;
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            //string _not = " تحويل من " + dr_store_from.Text + " الي " + dr_store_to.Text;
            setdata();
            ...
            foreach (DataGridViewRow item in gv_InvoiceDetails.Rows)
            {
                list_store_Log.Add(new store_log
                {
                    ...
                    nots = " تحويل صادر من " + item.Cells["Column2"].FormattedValue.ToString() + " الي " + dr_store_to.Text,
 ...
 list_store_Log.Add(new store_log
                {
                    ...
                    nots = " تحويل وارد من " + (item.Cells["Column2"] as DataGridViewComboBoxCell).FormattedValue.ToString() + " الي " + dr_store_to.Text,
                    DateAdd = invoiceHeader.DateAdd,
                    Source_Id = invoiceHeader.id,
                    id_type = invoiceHeader.id_invoice_type,
                    store_id = Session.ConvertInt(dr_store_to.SelectedValue.ToString()),
                    id_fara = invoiceHeader.id_fara,
                    id_user = invoiceHeader.id_user,
                });
            }
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            ...
            {
                var _prod = db.product_serch_Views.Where(x => (x.id == item.ItemID) && (x.store_id == invo_store_id)).FirstOrDefault();
                if (_prod != null)
                {
        ...
        private void btn_print_Click(object sender, EventArgs e)
        {
            ...
            para.Add(new ReportParameter("P_titel", "بيان تحويل من مخزن الي مخزن"));
        ...
        private void btn_add_grid_Click(object sender, EventArgs e)
        {
            ...
            {
                errorProvider1.SetError(tx_ItemQty, "الكمية المحوله اكبر من رصيد المخزن");
                return;
            }
        ...
        private bool chek_befor_delete_row(int row)
        {
            ...
            if (invoiceHeader.id != 0)
            {
                ...
                var Balance = db.product_serch_Views.Where(x => (x.id == ItemID) && (x.store_id == invo_store_id)).FirstOrDefault().Balance;
                if (ItemQty > Balance)
                {
